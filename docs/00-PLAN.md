...
### ✅ P3 — Asistente de inicio (onboarding de 3 pasos) (hecho 2026-09-24; primero se había saltado)
...
Modal `#mAsistente` (`js/asistente.js`, `css/asistente.css`): 1) rama (tarjetas con logo: Manada, Tropa, Comunidad, Clan; elegir avanza solo), 2) fechas (fin opcional → varios días), horas y lugar (por defecto Colegio La Salle Guaparo), 3) unidad o grupal (+ incluir Comunidad) y responsable, 4) «✏️ Crear manual» (aplica todo y abre 📋 Secciones) o «🤖 Importar JSON de Tico» (aplica y luego importa; el JSON manda), con «¿Cómo uso a Tico?» y el enlace al Gem. Valida rama y fecha de inicio. Aparece solo con la ficha vacía, no al abrir desde la biblioteca, y no vuelve tras completarlo o saltarlo (`ui_asistente_hecho`); se reabre desde 🧭 Asistente de inicio (barra lateral y «Más») y entonces trae lo que ya tiene la ficha. Lo elegido pasa por las funciones del panel de Configuración (código automático incluido). Verificado: camino manual (Grupal con Comunidad, 2 días, 09:00, lugar, responsable, código 031026-G), camino Tico, desde biblioteca no aparece, saltar no vuelve, Escape, 360 px sin scroll, axe 0 problemas y `verificar.mjs` sin cambios.
