...
- Hechos: P0, P1, **P2** (layout estilo GHL: barra lateral retráctil de alto completo + header morado + barra inferior en móvil) **P4** (panel derecho: Configuración en vivo con Deshacer + Programa en tarjetas arrastrables, días automáticos), **P5** (índice de secciones con progreso), **P6** (selector de I.L. con buscador/filtros/chips; ODS AA con íconos), **P7** (exportar solo con contenido), **P8** (Programas como biblioteca: manifiesto del repo + «Mi biblioteca» local + calendario), **P9** (17 erratas de I.L. aplicadas en la app) y **P10** (accesibilidad: axe 0 problemas, teclado y foco; impresión sin hojas vacías ni desbordes). **P3** (asistente de inicio; primero se saltó y Danny lo pidió después). **La Versión 1 del plan está completa (P0–P10)**; lo siguiente es la Versión 2 (solo documentada).
...
| `js/asistente.js` + `css/asistente.css` | P3: asistente de inicio `#mAsistente` (rama · fecha y lugar · unidad o grupal · «Crear manual» / «Importar JSON de Tico»). Solo con la ficha vacía y la primera vez (clave `ui_asistente_hecho`, fuera de `sf_*`); se reabre desde 🧭 en el menú. Aplica lo elegido con las funciones de `config-panel.js` |
